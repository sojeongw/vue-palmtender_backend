...
module.exports = new LocalStrategy({
        ...
        var paramName = req.body.userName || req.query.userName;
	 
		console.log('passport의 local-signup 호출됨 : ' + user_id + ', ' + password + ', ' + paramName);
		
	    // findOne 메소드가 blocking되지 않도록 하고 싶은 경우, async 방식으로 변경
	    process.nextTick(function() {
	    	var database = req.app.get('database');
		    database.userModel.findOne({ 'user_id' :  user_id }, function(err, user) {
		        // 에러 발생 시
		        if (err) {
		            return done(err);
		        }
		        ...
		        if (user) {
		        	...
		        	// 모델 인스턴스 객체 만들어 저장
		        	var user = new database.userModel({'user_id':user_id, 'password':password, 'userName':paramName});
